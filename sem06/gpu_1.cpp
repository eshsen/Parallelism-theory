...
Params parse_args(int argc, char** argv) {
    Params p;
    po::options_description desc("gpu1");
    desc.add_options()
        ("help,h", "print help")
...
int main(int argc, char** argv) {
    ...
    for (iter = 0; iter < p.max_iter; ++iter) {
        const bool check_error =
            ((iter + 1) % p.check == 0) || (iter + 1 == p.max_iter);
        ...
        if (check_error) {
            ...
            if (cur_a) {
                ...
                for (int j = 1; j < n - 1; ++j) {
                    for (int i = 1; i < m - 1; ++i) {
                        const std::size_t id = idx(j, i, m);
                        const double v = 0.25 * (buf_b[idx(j, i+1, m)] +
                                                 buf_b[idx(j, i-1, m)] +
                                                 buf_b[idx(j-1, i, m)] +
                                                 buf_b[idx(j+1, i, m)]);
                        buf_a[id] = v;
                        double diff = std::fabs(v - buf_b[id]);
                        if (diff > error) error = diff;
                    }
                }
            }
        ...
        cur_a = !cur_a;
        if (check_error && error <= p.tol) { ++iter; break; }
    }
